...
const WorkingTimeForm = (props) => {
    ...
    const [workingHoursOnChangeEmployee, setWorkingHoursOnChangeEmployee] =
        useState([]);
    ...
    const getWorkingHoursHandler = async () => {
        const api = await getWorkingHours(
            ...
            .then((res) => {
                const getWorkingHoursData = getResponseData(res);
    ...
    const onSubmit = (e) => {
        ...
        const formData = {
            NewWorkingHours: props.workingTimeFormInput
                .filter(
                    (hour) =>
                        (!hour.firstStartHour.includes('--') &&
                            !hour.firstEndHour.includes('--')) ||
                ...
                .map((hour) => {
                    ...
                    return {
                        ...
                        FirstStartHour:
                            hour.idAbsenceType !== 0
                                ? 0
                                : timeFormatHandler(hour, 'firstStartHour'),
                        FirstEndHour:
                            hour.idAbsenceType !== 0
                                ? 0
                                : timeFormatHandler(hour, 'firstEndHour'),
                        SecondStartHour: timeFormatHandler(
                            hour,
                            'secondStartHour',
                        ),
            ...
            for (let i = 0; i < 7; i++) {
                ...
                if (dateFromServer === undefined) {
                    ...
                    defaultForm.push({
                        ...
                        idAbsenceType: dateFromServer?.idAbsenceType
                            ? dateFromServer.idAbsenceType
                            : 0,
                        date: {
                        ...
                        firstStartHour:
                            dateFromServer?.idAbsenceType > 0
                                ...
                                : updateWorkingTimefromServer(
                                      dateFromServer.firstStartHour,
                                  ),
                        firstEndHour:
                            dateFromServer?.idAbsenceType > 0
                                ? '--:--'
                                : updateWorkingTimefromServer(
                                      dateFromServer.firstEndHour,
                                  ),
                        secondStartHour:
                            dateFromServer?.idAbsenceType > 0
                                ? '--:--'
                                : updateWorkingTimefromServer(
                                      dateFromServer?.secondStartHour,
                                  ),
                        secondEndHour:
                            dateFromServer?.idAbsenceType > 0
                                ? '--:--'
                                : updateWorkingTimefromServer(
                                      dateFromServer?.secondEndHour,
                                  ),
    ...
    const updateWorkingTimefromServer = (time) => {
        if (time === 0 || !time) {
            ...
            return `${formatHoursFromServer(time)}:${formatMinutesFromServer(
                time,
            )}`;
    };
        ...
        if (props.employeeId) {
            for (let i = 0; i < workingHoursOnChangeEmployee.length; i++) {
                defaultForm.push({
                    ...
                    idAbsenceType:
                        workingHoursOnChangeEmployee[i].idAbsenceType,
                    date: {
                    ...
                    firstStartHour:
                        workingHoursOnChangeEmployee[i].firstStartHour,
                    firstEndHour: workingHoursOnChangeEmployee[i].firstEndHour,
                    secondStartHour:
                        workingHoursOnChangeEmployee[i].secondStartHour,
                    secondEndHour:
                        workingHoursOnChangeEmployee[i].secondEndHour,
                    cellsDuration:
                        workingHoursOnChangeEmployee[i].cellsDuration,
                });
            }
            props.setWorkingTimeFormInput(defaultForm);
        }
    ...
    const employeesPreview = () => {
        const listItems = props.listOfEmployees
            .filter((user) =>
                user.serviceProviderId.includes(props.serviceProviderId),
            )
        ...
        return (
            ...
            >
                ...
                <div className={classes.WorkingTimeHeader}>
                    ...
                    <Select
                        name="monday"
                        onChange={(e) => weekSelectHandler(e)}
                    >
                        {mondays.map((monday, i) => {
                            return (
                                <option
