...
const EmployeesPicker = (props) => {
    ...
    const checkedEmployeesList = props.listOfEmployees.map((employee, i) => {
        return (
            <tr key={`${employee.id}${i}`} style={{ display: !props.editMode ? 'none' : 'flex' }}>
                <td>{employee.name}</td>
                <td
                    className={classes.IconTdEmployeesPicker}
                    onChange={(e) => checkBoxGroupToArrayHandler(e, props.checkedEmployees, props.setCheckedEmployees)}
                >
                    <CheckBox
